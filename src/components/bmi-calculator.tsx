  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-6 py-20 flex flex-col lg:flex-row items-center justify-center gap-16 lg:gap-32 animate-in fade-in duration-1000">
      ...
      <div className="w-full max-w-sm space-y-16">
        <h1 className="text-5xl font-black text-slate-950 tracking-tighter">Cek <span style={{ color: analysis.color }} className="transition-colors duration-500">BMI.</span></h1>
          ...
          {[
            ...
            <div key={item.id} className="space-y-5">
              ...
              <input 
                ...
                className="w-full h-2 bg-slate-100 rounded-full appearance-none cursor-pointer accent-slate-950" 
              />
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-col items-center max-w-md w-full">
         <div className="relative flex items-center justify-center mb-12">
            <div className="absolute w-80 h-80 rounded-full opacity-10 blur-[80px] transition-colors duration-700" style={{ backgroundColor: analysis.color }}></div>
         ...
         {/* Tingkatan BMI */}
         <div className="w-full grid grid-cols-6 gap-1 pt-8 border-t border-slate-100">
            {categories.map((cat) => (
              <div key={cat.label} className="flex flex-col items-center gap-2">
                 <div 
                  className={cn(
                    "w-full h-1.5 rounded-full transition-all duration-500",
                    analysis.label === cat.label ? "scale-y-150" : "opacity-20"
                  )} 
                  style={{ backgroundColor: cat.color }} 
                 />
                  <span className={cn(
                    "text-[9px] font-black uppercase tracking-tighter text-center transition-all",
                    analysis.label === cat.label ? "text-slate-950" : "text-slate-600"
                 )}>
                   {cat.label}
                 </span>
              </div>
